...
<div class="mb-6 sm:mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
    <div>
        <h1 class="text-2xl sm:text-3xl font-bold text-[#2C2416]">My Published Articles</h1>
        <p class="text-sm text-gray-500 mt-1">Manage your wellness articles, edit content, and publish new resources.</p>
    </div>
    
    <a href="{{ route('counselor.articles.create') }}" class="flex items-center gap-2 bg-[#C4A840] hover:bg-[#7B6B35] text-white px-5 py-2.5 rounded-lg font-semibold transition-colors shadow-sm">
        <i class="bi bi-pencil-square"></i> Write New Article
    </a>
</div>

<div class="bg-white rounded-2xl p-4 border border-[#C4A840]/20 shadow-sm mb-6 flex flex-col lg:flex-row gap-4 justify-between items-center relative z-20">
    ...
    <div class="relative w-full lg:w-96 shrink-0">
        ...
        <input type="text" class="w-full pl-10 pr-4 py-2.5 rounded-lg border border-gray-200 focus:border-[#C4A840] focus:ring-2 focus:ring-[#C4A840]/20 outline-none text-sm transition-colors" placeholder="Search articles by title or keywords...">
    </div>
    ...
    <div class="w-full lg:w-auto flex flex-col sm:flex-row gap-3">
        ...
        <div class="relative w-full sm:w-auto" x-data="{ sortOpen: false, selectedSort: 'Latest' }" @click.outside="sortOpen = false">
            <button @click="sortOpen = !sortOpen" class="w-full sm:w-44 flex justify-between items-center border border-gray-200 rounded-lg px-4 py-2.5 text-sm outline-none hover:border-[#C4A840] bg-white transition-colors">
            ...
            <div x-show="sortOpen" style="display: none;" class="absolute right-0 mt-2 w-full sm:w-48 bg-white border border-gray-100 rounded-lg shadow-lg z-50 py-1 overflow-hidden" x-transition>
                ...
                <button @click="selectedSort = 'Most Viewed'; sortOpen = false" :class="selectedSort === 'Most Viewed' ? 'bg-[#F5EFE0] text-[#7B6B35] font-semibold' : 'text-gray-700 hover:bg-gray-50'" class="w-full text-left px-4 py-2 text-sm transition-colors">Most Viewed</button>
        ...
        <div class="relative w-full sm:w-auto" x-data="{ filterOpen: false }" @click.outside="filterOpen = false">
            <button @click="filterOpen = !filterOpen" class="bg-gray-50 border border-gray-200 text-gray-600 hover:bg-[#F5EFE0] hover:text-[#7B6B35] hover:border-[#C4A840]/30 w-full sm:w-auto px-4 py-2.5 rounded-lg text-sm font-semibold transition-colors flex items-center justify-center gap-2">
            ...
            <div x-show="filterOpen" style="display: none;" class="absolute right-0 mt-2 w-full sm:w-64 bg-white border border-gray-100 rounded-lg shadow-xl z-50 p-4" x-transition>
                ...
                <select class="w-full px-3 py-2 rounded-lg border border-gray-200 focus:border-[#C4A840] outline-none bg-white text-sm text-gray-600 transition-colors">
...
<div class="bg-white rounded-2xl border border-[#C4A840]/20 shadow-sm overflow-hidden flex flex-col">
    ...
    <div class="bg-[#F5EFE0] px-6 py-4 border-b border-[#C4A840]/20 hidden md:grid grid-cols-12 gap-4 text-[#7B6B35] font-bold uppercase text-xs tracking-wider shrink-0 z-10">
        ...
        @forelse($articles as $article)
            <div class="p-4 sm:px-6 sm:py-5 hover:bg-gray-50 transition-colors flex flex-col md:grid md:grid-cols-12 md:items-center gap-4">
                <div class="col-span-6">
                    <h3 class="text-lg font-bold text-[#2C2416] mb-1">{{ $article->title }}</h3>
                    ...
                    <div class="flex items-center gap-3 mt-2 text-xs text-gray-400 font-medium">
                        ...
                        <span>•</span>
                        <span class="text-[#C4A840] bg-[#C4A840]/10 px-2 py-0.5 rounded">{{ $article->category }}</span>
                    </div>
                </div>
                ...
                <div class="col-span-2 flex justify-start md:justify-end gap-2">
                    <a href="#" class="p-2 text-gray-400 hover:text-[#C4A840] hover:bg-[#F5EFE0] rounded-lg transition-colors" title="Edit Article">
