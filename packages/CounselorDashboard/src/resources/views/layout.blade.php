...
<body class="bg-[#F5EFE0] text-[#2C2416] font-[Lato,sans-serif] antialiased flex flex-col h-screen overflow-hidden" x-data="{ isModalOpen: false }" x-data="{ sidebarOpen: true }">
    
    <header class="bg-[#FDFAF4] border-b border-[#C4A840]/20 shadow-sm flex items-center justify-between px-6 py-3 z-20 shrink-0 relative" @open-session-report.window="isModalOpen = true">
        ...
        <div class="relative" x-data="{ open: false }">
            <button @click="open = !open" @click.away="open = false" class="flex items-center gap-2 focus:outline-none hover:bg-[#F5EFE0] px-3 py-1.5 rounded-lg transition-colors">
                <img src="https://ui-avatars.com/api/?name={{ urlencode(session('counselor_name', 'Counselor')) }}&background=C4A840&color=fff" alt="Profile" class="h-8 w-8 rounded-full border border-[#C4A840]/30">
            ...
            <div x-show="open" style="display: none;" class="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-gray-100 py-1 z-50" x-transition>
                <a href="{{ url('') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-[#F5EFE0] hover:text-[#7B6B35]"><i class="bi"></i> Back to main page</a>
    ...
    <div class="flex flex-1 overflow-hidden">
        <aside :class="sidebarOpen ? 'ml-0' : '-ml-64'" class="w-64 bg-white border-r border-[#C4A840]/20 flex-shrink-0 flex flex-col h-full z-10 shadow-[2px_0_10px_rgba(0,0,0,0.02)] transition-all duration-300 ease-in-out">
            <nav class="flex-1 overflow-y-auto py-6 px-4 space-y-2">
                ...
                <button @click="$dispatch('open-session-report')" class="w-full mt-4 flex items-center justify-center gap-2 px-3 py-2.5 rounded-lg bg-gray-50 text-gray-600 border border-gray-200 hover:bg-[#F5EFE0] hover:text-[#7B6B35] hover:border-[#C4A840]/30 font-medium transition-all">
            ...
            <div class="fixed inset-0 z-10 w-screen overflow-y-auto">
                <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                    ...
                    <div x-show="isModalOpen" 
                         ...
                         class="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-xl border border-[#C4A840]/30">
                        ...
                        <div class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                            <div class="sm:flex sm:items-start">
                                <div class="mx-auto flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full bg-[#F5EFE0] sm:mx-0 sm:h-10 sm:w-10">
                                    <i class="bi bi-file-earmark-text text-[#7B6B35] text-xl"></i>
                                </div>
                                <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left w-full">
                                    <h3 class="text-lg font-bold leading-6 text-[#2C2416]" id="modal-title">Submit Session Report</h3>
                                    <div class="mt-2">
                                        ...
                                        <form class="space-y-4 text-left">
                                            ...
                                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                                <div>
                                                    ...
                                                    <select class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-[#C4A840] focus:ring focus:ring-[#C4A840]/20 p-2 border bg-white text-sm">
                                                ...
                                                <div>
                                                    ...
                                                    <input type="text" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-[#C4A840] focus:ring focus:ring-[#C4A840]/20 p-2 border text-sm" placeholder="e.g. Academic Anxiety">
                                                </div>
                                            </div>

                                            <div>
                                                <label class="block text-sm font-semibold text-gray-700">Clinical Notes & Observations</label>
                                                <textarea rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-[#C4A840] focus:ring focus:ring-[#C4A840]/20 p-2 border text-sm" placeholder="Document client's emotional state, general topics discussed, and progress..."></textarea>
                                            </div>

                                            <div>
                                                <label class="block text-sm font-semibold text-gray-700">Interventions Used</label>
                                                <input type="text" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-[#C4A840] focus:ring focus:ring-[#C4A840]/20 p-2 border text-sm" placeholder="e.g., Cognitive Restructuring, 4-7-8 Breathing...">
                                            </div>

                                            <div>
                                                <label class="block text-sm font-semibold text-gray-700">Next Steps / Homework</label>
                                                <textarea rows="2" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-[#C4A840] focus:ring focus:ring-[#C4A840]/20 p-2 border text-sm" placeholder="List any actionable tasks or habits for the client to practice..."></textarea>
                                            </div>
                        ...
                        <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6 border-t border-gray-100">
                            <button type="button" @click="isModalOpen = false" class="inline-flex w-full justify-center rounded-md bg-[#C4A840] px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-[#7B6B35] sm:ml-3 sm:w-auto transition-colors">Save Report</button>
