...
<head>
    ...
    <title>{{ ucfirst($role) }} Sign In - Werta</title>
    ...
    <div class="w-full max-w-md mx-4">
        ...
        <div class="bg-white rounded-2xl shadow-lg border border-[#C4A840]/20 p-8">
            <div class="flex items-center justify-center gap-2 mb-2">
                <i class="bi bi-shield-lock text-[#C4A840] text-xl"></i>
                <span class="text-xs font-bold uppercase tracking-widest text-[#7B6B35]">{{ ucfirst($role) }} Portal</span>
            </div>
            <h2 class="text-2xl font-bold text-[#2C2416] text-center mb-2">Staff Sign In</h2>
            <p class="text-sm text-gray-500 text-center mb-6">Restricted access. Authorized {{ $role }} personnel only.</p>
            ...
            <form method="POST" action="{{ url('/' . $role . '/login') }}" class="space-y-5">
                @csrf
                <div>
